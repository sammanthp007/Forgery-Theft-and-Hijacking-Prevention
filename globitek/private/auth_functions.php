...
function log_in_user($user) {
    // DONE Store user's ID in session
    $_SESSION['user_id'] = $user['id'];
    // DONE Store last login time in session
    $_SESSION['last_login'] = time();
    $_SESSION['logged_in'] = true;
    return true;
}

// DONE Regenerate the session ID at the appropriate point
...
function destroy_current_session() {
    // DONE destroy the session file completely
    session_unset();
    session_destroy();
}
...
function last_login_is_recent() {
    // DONE add code to determine if last login is recent
    $session_limit = 86400;
    if (!isset($_SESSION['last_login'])) { return false; }
    return ($_SESSION['last_login'] + $session_limit >= time());
}
